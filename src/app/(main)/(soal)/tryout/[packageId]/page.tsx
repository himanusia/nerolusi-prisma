...
export default function QuizPage() {
  ...
  const getSubtestDisplayName = (type) => {
    switch (type) {
      case "pu": return "Kemampuan Penalaran Umum";
      case "ppu": return "Pengetahuan dan Pemahaman Umum";
      case "pbm": return "Kemampuan Memahami Bacaan dan Menulis";
      case "pk": return "Pengetahuan Kuantitatif";
      case "lbi": return "Literasi dalam Bahasa Indonesia";
      case "lbe": return "Literasi dalam Bahasa Inggris";
      case "pm": return "Penalaran Matematika";
      default: return type;
    }
  };
  ...
  return isError ? (
    ...
    <div className="flex min-h-screen bg-white">
      {/* Left Panel */}
      <div className="md:w-1/3 w-full px-10 py-5 border-[1px] border-[#acaeba]">
        {/* Back Button */}
        <Button 
          variant="outline" 
          ...
          {sortedSubtests?.map((subtest, index) => {
            ...
            return (
              <Button
                ...
                className={`w-full h-auto min-h-10 justify-center items-center rounded-lg transition-all whitespace-normal ${
      ...
      {/* Right Panel */}
      <div className="w-2/3 bg-[#2b8057]/30 hidden md:flex items-center justify-center p-8">
        ...
        <DialogContent className="sm:max-w-[425px] max-w-[350px]">
          <DialogHeader>
            <DialogTitle className="text-center">
              {selectedSubtest && (() => {
                const isSubmitted = selectedSubtest.quizSession && new Date(selectedSubtest.quizSession) <= new Date();
                const subtestName = getSubtestDisplayName(selectedSubtest.type);

                if (isSubmitted) {
                  const isPackageEndDatePassed = new Date(packageData.TOend) < new Date();
                  return isPackageEndDatePassed ? <p className="text-2xl font-bold text-black">Lihat Pembahasan</p> : <p className="text-2xl font-bold text-black">Subtest Selesai</p>;
                } else {
                  return (
                    ...
                    <div>
                      ...
                      {selectedSubtest.score && (
                        <p className="text-lg font-semibold text-green-600">
                          Score: {selectedSubtest.score} | {selectedSubtest.totalCorrect}/{selectedSubtest.totalQuestion}
                        </p>
                      )}
                    </div>
                ...
                return (
                  <>
                    <div>
                      ...
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-semibold">{selectedSubtest.duration} Menit</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <Button 
                      ...
                      onClick={() => {
                        setDialogOpen(false);
                        handleSubtestClick(selectedSubtest.id, selectedSubtest.duration);
                      }}
                    >
                      Start!
                    </Button>
